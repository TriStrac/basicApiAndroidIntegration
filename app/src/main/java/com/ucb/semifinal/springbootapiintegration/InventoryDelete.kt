...
import com.ucb.semifinal.springbootapiintegration.databinding.ActivityInventoryDeleteBinding
import com.ucb.semifinal.springbootapiintegration.network.RetrofitClient
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class InventoryDelete : AppCompatActivity() {
    private lateinit var binding:ActivityInventoryDeleteBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityInventoryDeleteBinding.inflate(layoutInflater)
    ...
    private fun deleteAccount(email: String) {
        RetrofitClient.apiService.deleteInventoryByEmail(email).enqueue(object : Callback<String> {
            override fun onResponse(call: Call<String>, response: Response<String>) {
                if (response.isSuccessful) {
                    ...
                    when (response.code()) {
                        404 -> binding.loginStatus.text="Inventory account not found"
                        else -> binding.loginStatus.text="Failed to delete Inventory: ${response.code()}"
                    }
                }
            }
            ...
            override fun onFailure(call: Call<String>, t: Throwable) {
                ...
                binding.loginStatus.text="Inventory Deleted Successfully"
            }
        })
    }
}